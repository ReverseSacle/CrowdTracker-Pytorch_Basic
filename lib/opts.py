...
class opts(object):
  def __init__(self,current_dir,model_path,input_path,threshold,match_threshold,use_gpu,task='mot'):
    self.task = task
    ...
    self.parser.add_argument('--load_model', default=model_path,
    ...
    # system
    self.parser.add_argument('--gpus', default=use_gpu,
    ...
    self.parser.add_argument('--input-video', type=str,default=input_path,
                             help='path to the input video')
    self.parser.add_argument('--output_format', type=str, default='video', help='video or text')
    self.parser.add_argument('--output_root', type=str, default=current_dir + '/output_result', help='expected output root path')
  ...
  def parse(self, args=''):
    ...
    opt.exp_dir = os.path.join(opt.root_dir, 'exp', task)
  ...
  def init(self, args=''):
    ...
    opt = self.parse(args)
    task = self.task
    dataset = Struct(default_dataset_info[task])
