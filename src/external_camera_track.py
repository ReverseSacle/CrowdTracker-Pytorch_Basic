...
def external_camera_tracker(threshold_value, root_dir, use_gpu,
    ...
    try:
        ...
        if cap_test is None or not cap_test.isOpened():
            msg_box = QMessageBox(QMessageBox.Warning, 'Warning！', 'The external camera is not exit')
            msg_box.exec_()
        else:
            cap_test.release()
        ...
        output_video_dir = root_dir + '/output_external_camera'
